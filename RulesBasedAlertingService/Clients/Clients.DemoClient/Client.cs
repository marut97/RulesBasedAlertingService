...
using Clients.DemoClient.VitalsDataService;
using Models.Doctor;
using Models.Patient;
using Models.Device;
using Models.PatientVitals;
using Newtonsoft.Json;
using AdmissionHistory = Models.Patient.AdmissionHistory;
using Device = Models.Device.Device;
using Doctor = Models.Doctor.Doctor;
using HospitalBed = Models.HospitalBed.HospitalBed;
using Limits = Models.Device.Limits;
using Patient = Models.Patient.Patient;
using PatientAdmission = Models.PatientAdmission.PatientAdmission;
    ...
    public class Client
    {
        ...
        static void Main(string[] args)
        {
            ...
            for (int i = 0; i < 20; i++)
            {
                ...
                vitals.Vitals = new List<Vitals>() {new Vitals {DeviceId = "Temperature", Value = 85}};
                client.WriteVitals(vitals);
                Thread.Sleep(2000);
                vitals.PatientId = "222";
                vitals.Vitals = new List<Vitals>() {new Vitals { DeviceId = "Temperature", Value = 20 }, new Vitals {DeviceId = "SPO2", Value = 78}};
                Thread.Sleep(2000);
                client.WriteVitals(vitals);
            }
        }
        ...
        private static void HospitalBedRegistration()
        {
            ...
            json = "{\"beds\":" + json + "}";
            client.Headers[HttpRequestHeader.ContentType] = "application/json";
            var response = client.UploadString(url + "RegisterBeds", "PUT", json);
        }
        ...
        private static void DoctorRegistration()
        {
            ...
            json = "{\"doctor\":" + json + "}";
            client.Headers[HttpRequestHeader.ContentType] = "application/json";
            var response = client.UploadString(url + "RegisterDoctor", "PUT", json);
            ...
            json = "{\"doctor\":" + json + "}";
            client.Headers[HttpRequestHeader.ContentType] = "application/json";
            response = client.UploadString(url + "RegisterDoctor", "PUT", json);
        ...
        private static void PatientRegistration()
        {
            ...
            json = "{\"patient\":" + json + "}";
            client.Headers[HttpRequestHeader.ContentType] = "application/json";
            var response = client.UploadString(url + "RegisterPatient", "PUT", json);
            ...
            json = "{\"patient\":" + json + "}";
            client.Headers[HttpRequestHeader.ContentType] = "application/json";
            response = client.UploadString(url + "RegisterPatient", "PUT", json);
